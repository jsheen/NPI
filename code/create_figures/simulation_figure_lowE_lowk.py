...
with open(home + '/NPI/code_output/figs/sims_con_lowE_lowk.pickle', 'wb') as handle:
    pickle.dump(sims_con, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open(home + '/NPI/code_output/figs/sims_trt_lowE_lowk.pickle', 'wb') as handle:
...
for i in range(len(I_cons)):
    ...
    ax1.set_xlabel('Days since start of epidemic')
...
plt.tight_layout()
plt.savefig(home + '/NPI/code_output/figs/sim_fig_bottom.png', dpi=300)
