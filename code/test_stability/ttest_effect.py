...
def get_effect(It_It1con_It1trt, cluster_num, nsample):
    ...
    for sim_i in range(nsim):
        ...
        for con_dex in range(ncluster):
            ...
            log_It1It_con = np.log((sampled_It1_con + 0.25) / (sampled_It_con + 0.25))
        ...
        for trt_dex in range(ncluster, ncluster * 2):
            ...
            log_It1It_trt = np.log((sampled_It1_trt + 0.25) / (sampled_It_trt + 0.25))
...
for param_set in param_sets:
    ...
    if ncluster != -1 and ncluster != 1000:
        ...
        with open(home + '/Desktop/temp/res_' + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + '_ttest_effect_0.25.pickle', 'wb') as handle:
...
for param_set in param_sets:
    ...
    param_res_05 = pd.read_pickle(home + '/Desktop/temp/res_' + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + '_ttest_effect_0.5.pickle')
    param_res_025 = pd.read_pickle(home + '/Desktop/temp/res_' + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + '_ttest_effect_0.25.pickle')
    new_row = []
    ...
    if (len(param_res) > 1):
        ...
        new_row.append('-')
    if (len(param_res_05) > 1):
        new_row.append(np.mean(param_res_05[2]))
    else:
        new_row.append('-')
    if (len(param_res_025) > 1):
        new_row.append(np.mean(param_res_025[2]))
    else:
        new_row.append('-')
    to_pd.append(new_row)
df = pd.DataFrame(to_pd, columns = ['R0', 'N', 'k', 'true effect', 'n sample', 'diff. of means +1', 'diff. of means +0.5', 'diff. of means +0.25']) 
