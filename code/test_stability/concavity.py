...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  2 20:21:12 2021

@author: Justin Sheen

@description: check concavity of solved traces for those that need over 500 clusters.
...
for param_set in param_sets:
    ...
    if len(pickle_res) > 2 and float(pickle_res[-1].split(',')[1]) > 500:
        xs = []
        ys = []
        for i in pickle_res:
    ...
    if len(pickle_res) > 2 and float(pickle_res[-1].split(',')[1]) > 500:
        ...
        plt.xlim(500, 1000)
        plt.ylim(0.45, 1)
plt.plot([1, 1000], [0, 0.8], 'k-', color = 'r', linestyle='dashed')
plt.xlabel('Number of clusters per arm')
plt.ylabel('Power')
plt.tight_layout()
plt.savefig(home + '/NPI/code_output/figs/concavity.png', dpi=300)
# Plot slopes -----------------------------------------------------------------
#plt.hist(slopes)
