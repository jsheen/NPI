# -----------------------------------------------------------------------------
# @description: script used to create the figure showing effect of sensitivity analysis
# -----------------------------------------------------------------------------
...
require(gridExtra)
require(grid)
lb <- read.csv("~/NPI/code_output/tables/ttest_deg8.csv", stringsAsFactors = F)
lb2 <- read.csv("~/NPI/code_output/tables/ttest.csv", stringsAsFactors = F)
lb3 <- read.csv("~/NPI/code_output/tables/ttest_deg30.csv", stringsAsFactors = F)
param_sets <- list()
param_sets_dex <- 1
R0s <- c(1.5, 2)
ns <- c(1000, 10000)
...
for (n in ns) {
  ...
  helper_function <- function(toplot, k, n) {
    ...
    if (n == 10000 & k == 0.1) {
      ...
      subplot_20_2 <- rid_single_points(subplot_20_2)
      subplot_final <- subplot_final + geom_point(size=4, subplot_20_2, mapping=aes(x=x,y=value,group=variable,color='20'))
      subplot_40_2 <- rid_single_points(subplot_40_2)
      subplot_final <- subplot_final + geom_point(size=4, subplot_40_2, mapping=aes(x=x,y=value,group=variable,color='40'))
      ...
      subplot_final <- ggplot() + 
        ...
        facet_grid(. ~ title)+
        scale_color_manual(values=c('steelblue1', 'steelblue4'))
      subplot_20_1.5 <- rid_single_points(subplot_20_1.5)
      subplot_final <- subplot_final + geom_point(size=4, subplot_20_1.5, mapping=aes(x=x,y=value,group=variable,color='20'))
      subplot_20_2 <- rid_single_points(subplot_20_2)
      subplot_final <- subplot_final + geom_point(size=4, subplot_20_2, mapping=aes(x=x,y=value,group=variable,color='20'))
      subplot_40_1.5 <- rid_single_points(subplot_40_1.5)
      subplot_final <- subplot_final + geom_point(size=4, subplot_40_1.5, mapping=aes(x=x,y=value,group=variable,color='40'))
      subplot_40_2 <- rid_single_points(subplot_40_2)
      subplot_final <- subplot_final + geom_point(size=4, subplot_40_2, mapping=aes(x=x,y=value,group=variable,color='40'))
...
ggsave(filename="~/NPI/code_output/figs/Sheen_FigS3.tiff", marrangeGrob(grobs = ps, nrow=4, ncol=1, top=NULL),
       width=13, height=16, units='in', dpi=600)
ggsave(filename="~/NPI/code_output/figs/Sheen_FigS3.eps", marrangeGrob(grobs = ps, nrow=4, ncol=1, top=NULL),
       width=13, height=16, units='in', dpi=600)
ggsave(filename="~/NPI/code_output/figs/Sheen_FigS3.png", marrangeGrob(grobs = ps, nrow=4, ncol=1, top=NULL),
...
ggsave(filename="~/NPI/code_output/figs/Sheen_FigS3_legend.tiff", mylegend,
       width=12, height=1, units='in', dpi=600)
ggsave(filename="~/NPI/code_output/figs/Sheen_FigS3_legend.png", mylegend,
