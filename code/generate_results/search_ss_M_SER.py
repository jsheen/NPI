...
def get_power(It_It1con_It1trt, cluster_num, nsample):
    ...
    for sim_i in range(nsim):
        ...
        for clus_dex in range(ncluster * 2):
            ...
            sampled_Et = sampled_t[1]
            sampled_It = sampled_t[2]
            sampled_Rt = sampled_t[3]
            sampled_SERt = sampled_St + sampled_Et + sampled_Rt
            if sampled_SERt != (nsample - sampled_It):
                raise NameError("Should be equal.")
            ...
            SERt_ItIt1.append([sampled_SERt, log_It1It_con, log_It1It_trt])
        SERt_ItIt1_con_trt = []
        while len(SERt_ItIt1) != 0:
            dists = [abs(SERt_ItIt1[0][0] - x[0]) for x in SERt_ItIt1[1:]]
            index_clus2 = dists.index(min(dists)) + 1
            SERt_ItIt1_con_trt.append([SERt_ItIt1[0], SERt_ItIt1[index_clus2]])
...
for param_set in param_sets:
    ...
    filename = home + "/NPI/code_output/res/res_" + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + "_M_SER.csv"
    with open(filename, 'w') as out_f:
        out_f.write(str(sufficient_ncluster))
    with open(home + '/NPI/code_output/res/res_' + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + '_M_SER.pickle', 'wb') as handle:
