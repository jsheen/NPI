...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 13 23:21:17 2021

@author: Justin Sheen

@description: sensitivity test for degree 30
"""
...
for param_set in param_sets:
    ...
    It_It1con_It1trt = pd.read_csv(home + "/NPI/code_output/res/" + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_deg30.csv", header=None)
    It_It1con_It1trt = It_It1con_It1trt.drop([3000])
    sufficient_ncluster, focal_iters = dac(It_It1con_It1trt, 1, 1000, curr_nsample, [])
    filename = home + "/NPI/code_output/res/res_" + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + "_ttest_deg30.csv"
    with open(filename, 'w') as out_f:
        out_f.write(str(sufficient_ncluster))
    with open(home + '/NPI/code_output/res/res_' + str(tgt_R0) + "_" + str(N_cluster) + "_" + str(k_overdispersion) + "_" + str(effect) + "_" + str(expected_It_N) + "_" + str(curr_nsample) + '_ttest_deg30.pickle', 'wb') as handle:
